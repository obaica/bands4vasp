...
SUBROUTINE WRITE_PROGRESS(S,N)
 INTEGER :: I,S,N,PROG,NBAR
    PROG=INT(0.5+REAL(S*100)/REAL(N))
...
!  READ(11,*) PAR%SION
  ...
  IF(SCAN(CHARA,'-')>0)THEN
    ...
    IF(PAR%SION(2)>0)THEN
      ...
      WRITE(*,'(A,I4,A,I4)') 'Take IONs ',PAR%SION(1),' to ',PAR%SION(2)
    ELSE
      PAR%NSION=1
      WRITE(*,'(A,I4)') 'Take ION ',PAR%SION(1)
      WRITE(*,'(A,I4)') 'ION position 2 was not in range :: ',PAR%SION(2)
    END IF
  ELSE
    PAR%SION(2)=0
    PAR%NSION=1
    READ(CHARA,'(I10)') PAR%SION(1)
    IF(PAR%SION(1)>0) WRITE(*,'(A,I4,A)') 'Take values of ION ',PAR%SION(1),' for calculation'
  END IF
  ...
  END IF
 CLOSE(UNIT=11)
 IF(PAR%FBORDER<PAR%WDIF)THEN
   WRITE(*,*) 'WARNING: The energyinterval for the unfiltered plot is less'
   WRITE(*,*) '         then the energyinterval for the filtered plot (INPAR)'
   WRITE(*,'(A,f9.5)') '  ==>    set both intervals to:',PAR%FBORDER
   PAR%WDIF=PAR%FBORDER
 END IF
...
SUBROUTINE READ_LATTICE(FILEDIR,LATTICE,LUNFOLD)
 IMPLICIT NONE
 TYPE (LATT) :: LATTICE
 INTEGER :: I,J,NSCALE,SCALEX,SCALEY,SCALEZ,IO_ERROR,LPOS
...
!Spin muss noch gecheckt/eingelesen werden
...
!     END DO spin
 ...
 IF(PAR%FORB>N%ORB)THEN
   ...
   & PAR%FORB, " is higher than the occuring Orbitals in PROCAR"
   WRITE(*,'(A,I4)') "Orbital set to ", N%ORB
   PAR%FORB=N%ORB
 END IF
...
SUBROUTINE READBODY_PROCAR_PRIM(FILEDIR,TEBS,KGRID,LATTICE,LUNFOLD,LFINISH)
 IMPLICIT NONE
 TYPE( BAND ), DIMENSION(:), ALLOCATABLE :: TEBS, TEMBS
 TYPE( LATT ) :: LATTICE
...
!                  print*,'cdum=',cdum
          ...
          IF ((ABS(ENERGY)<=(PAR%FBORDER+2.0_DP*PAR%SIGMA)).AND.(BLOCH>PAR%TRASH)) THEN
          IF((PAR%SION(1)>0).AND.(PAR%SION(1)<=NFILE(P)%IONS))THEN
            ...
            DO J=1,PAR%NSION
              ...
              DO ND=1,NFILE(P)%ORBIT
                ...
                ELSE
                  READ(UN,'(F6.3,1X)',ADVANCE='NO') SORBIT  !ORBITS(ND)  !read orbital characters
          ...
          END IF
           ...
           COL=COL+1
           IF(COL-1>600000)THEN
             PAR%FBORDER=PAR%FBORDER/2.0_DP
             WRITE(*,*) 'WARNING: The number of Points for the unfiltered plot &
             & is to high'
             WRITE(*,*) 'Set the energy interval to: ',PAR%FBORDER
             DEALLOCATE(TEMBS,KGRID)
             CLOSE(UN)
             CLOSE(9999)
             RETURN
           END IF
...
SUBROUTINE READ_PRJCAR(FILEDIR, TEBS, KGRID, LATTICE )
 ...
 CHARACTER(80) :: SCDUM
 CHARACTER(2) :: B1,B2,B3
 LOGICAL :: NEWPATH,STUCT,LEXIST,SLIM_PRJCAR,LKT,LDUM,LMINV,LKFIRST
 TOTKPT=0
 TOTDIST=0.0_DP
 UNARRAY=9924
         ...
         IF(CDUM(1:7)=='k-point')THEN
           ...
           bands: DO WHILE (BND<=NFILE(P)%BND)
             ...
             IF(CDUM(1:5)=='band:')THEN
               ...
               ENERGY=ENERGY-PAR%EFERMI
               IF(ABS(ENERGY)>PAR%FBORDER) THEN
                 BND=BND+1
                 READ(UN,'(A)',IOSTAT=IOCHECK) CDUM
                 CYCLE
               END IF
...
!                 IF((KAPPA_SUM>0.001_DP).AND.(PAR%NORM)) print*,'norm it baby',BLOCHS(NI),' : ',BLOCHS(NI)/KAPPA_SUM
     ...
     DO J=1,N%SAMEK
       IF(FOUNDK(J))THEN
         ...
         ELSE 
           ...
           WEIGHT=1.0_DP
         END IF
         IF(ABS(ENERGY(J))>PAR%WDIF)THEN!.OR.(WEIGHT(J)<=PAR%TRASH))THEN
...
LOGICAL FUNCTION WRITE_PLOTDATA(PLOTFILE,ACOLBAND,TEBS,FILETYPE,SPINFILE)
 IMPLICIT NONE
 TYPE( BAND ),DIMENSION(:),INTENT(IN) :: TEBS
 CHARACTER(LEN=*) :: PLOTFILE,ACOLBAND,SPINFILE
 CHARACTER(LEN=50) :: PFILE,SPFILE,BPFILE
 CHARACTER(LEN=1) :: CSPN
 INTEGER :: B, K, I,SPN,NBANDS, FILETYPE,OR,OBEGIN,OEND,ACOLEN
 INTEGER, DIMENSION(N%BAND) :: BNDINDEX
 LOGICAL, DIMENSION(N%ORB) :: NOORB
 SPIN: DO SPN=1,PAR%SPN
 IF(PAR%SPN==2)THEN
   WRITE(CSPN(1:1),'(I1)') SPN
   ACOLEN=LEN_TRIM(ACOLBAND)+2
   BPFILE=CSPN//TRIM(ACOLBAND)
   PFILE=TRIM(PLOTFILE)//CSPN
   SPFILE='SLIM'//TRIM(PLOTFILE)//CSPN
 ELSE
   ACOLEN=LEN_TRIM(ACOLBAND)+1
   BPFILE=TRIM(ACOLBAND)
   PFILE=TRIM(PLOTFILE)
   SPFILE='SLIM'//TRIM(PLOTFILE)
 END IF
 NOORB=.TRUE.
 BNDINDEX(:)=0
 NBANDS=0
...
!     IF(ANY(NOORB))THEN
       ...
       DO OR=1,N%ORB
        IF(NOORB(OR))CYCLE
        WRITE(65,'(A1,I2,A)') '#',I,'ORBITAL='//TRIM(ADJUSTL(PAR%ORBNAMES(OR)))
        I=I+1
       END DO
!     ELSE
!        WRITE(75,'(A)') '#noorbital'
!        WRITE(*,'(A)') 'Unfiltered data:: No orbitalcharcter greater than 0.01 found'
!     END IF
     WRITE(65,'(A,I3,",",I3)') '#LAYOUT=',GET_LAYOUT(I)
   ELSE
     I=1
     WRITE(65,'(A)') '# 1ORBITAL='//TRIM(ADJUSTL(PAR%ORBNAMES(PAR%FORB)))
     IF(NOORB(PAR%FORB))I=0
   END IF
   IF(I==0)THEN
     WRITE(65,'(A)') '#noorbital'
     IF(PAR%SPN==2)THEN
       WRITE(*,'(A,I1,A)') 'Unfiltered data (spin=',SPN,'):: No orbitalcharcter greater than 0.05 found'
     ELSE
       WRITE(*,'(A)') 'Unfiltered data:: No orbitalcharcter greater than 0.05 found'
     END IF
     WRITE(*,*) ''
   END IF
  END IF
   DO I=1,SIZE(TEBS)
     IF(TEBS(I)%LOCU.AND.(ABS(TEBS(I)%E)<PAR%FBORDER).AND.(TEBS(I)%SPN==SPN)) THEN
       IF(PAR%FORB==0)THEN
         ...
         END DO
         WRITE(65,'(1X,I4)') TEBS(I)%BND
       ELSE
         WRITE(65,'(4(F14.8),1X,I4)') TEBS(I)%K,TEBS(I)%E,&
         & TEBS(I)%W, TEBS(I)%OC(PAR%FORB), TEBS(I)%BND
       END IF
       IF(PAR%BANDPLOT) BNDINDEX=MAKE_BAND_LIST(BNDINDEX,TEBS(I)%BND,NBANDS)
     END IF
   END DO
 CLOSE(65)
 ...
 IF(PAR%BANDPLOT)THEN
   DO K=1,NBANDS
     ...
     END IF
     OPEN(UNIT=75,FILE=TRIM(BPFILE),STATUS='REPLACE',ACTION='WRITE')
   DO I=1,SIZE(TEBS)
   IF(TEBS(I)%LOCU.AND.(TEBS(I)%SPN==SPN))THEN
     IF((TEBS(I)%BND==B).AND.(ABS(TEBS(I)%E)<PAR%FBORDER)) THEN
...
END IF
 ...
 DO K=1,N%TKPTS
   DO I=1,N%SAMEK
     IF((EBS(K,I,SPN)%LOCU).AND.(ABS(EBS(K,I,SPN)%E)<PAR%WDIF))THEN
       IF(PAR%FORB==0)THEN
         DO OR=1,N%ORB
...
!    ELSE
!      WRITE(75,'(A)') '#noorbital'
!      WRITE(*,'(A)') 'Filtered data:: No orbitalcharcter greater than 0.01 found'
!    END IF
  ...
  IF(I==0)THEN
    WRITE(75,'(A)') '#noorbital'
    IF(PAR%SPN==2)THEN
      WRITE(*,'(A,I1,A)') 'Filtered data (spin=',SPN,'):: No orbitalcharcter greater than 0.05 found'
    ELSE
      WRITE(*,'(A)') 'Filtered data:: No orbitalcharcter greater than 0.05 found'
    END IF
    WRITE(*,*) ''
  END IF
END IF
...
!!!! Achtung hier wurde pi als 45 grad definiert
!!!! nochmal überprüfen
SUBROUTINE FINDFITTPOINTS(FP, LFIT, SPN)
 IMPLICIT NONE
 TYPE( BAND ) :: EPOINT
 TYPE( BAND ), ALLOCATABLE :: TFP(:),FP(:,:),STATFP(:,:)
 INTEGER :: I,J,K,P,NK,NJ,NA,KK,NFP,NCROS,NN,SPN
...
!   TKPTS=1
 DO P=1,N%PATH
   IF(N%PATH>=15) CALL WRITE_PROGRESS(P,N%PATH)
   kpoints: DO
...
!Ermittle den nachbarpunkt vom gleichen Band >> EPOINT
         LEXIST1=.TRUE.
         DO WHILE ( LEXIST1 )
           ...
           IF ( LEXIST1 ) THEN
             ...
             & .AND. (ORBIT_DIF(EBS(K,J,SPN)%OC(:),EPOINT%OC(:)))) THEN
               ...
               HITNUM(NA+1,:)=(/K+1,NJ/)
               TFP( NA+1 ) = EPOINT
               DIST(NA)=ABS(EPOINT%E+EBS(K,J,SPN)%E) 
               GRAD(NA)=(EPOINT%E-EBS(K,J,SPN)%E)!/(EPOINT%K-EBS(K,J,SPN)%K)
...
!
                   IF (LEXIST2) THEN
                     GRAD(NA-1)=(EPOINT%E-TFP(NA-1)%E)!/(EPOINT%K-TFP(NA-1)%K)
...
!***  with the value 'Energy' and DOR 'I' and saves it in 'NEBS' ******
...
!Get the outher points which where used for the linear regression
    ...
    IF (ALPHA(2)>=0)THEN
      PLOT(1,2)=MAX(MINVAL(FP(KA,:)%E),-PAR%WDIF)
      PLOT(2,2)=MIN(MAXVAL(FP(KA,:)%E),PAR%WDIF)
    ELSE
      PLOT(1,2)=MIN(MAXVAL(FP(KA,:)%E),PAR%WDIF)
      PLOT(2,2)=MAX(MINVAL(FP(KA,:)%E),-PAR%WDIF)
    END IF
...
SUBROUTINE WRITE_FERMIFILE(FERMIFILE,ROOTS,ROOTSXYZ,KGRID,LATTICE,FILETYPE)
  ...
  OPEN(UNIT=851, FILE='temp'//TRIM(FERMIFILE), STATUS='REPLACE', ACTION='WRITE')
...
SUBROUTINE ENERGYGAP( TEBS, EGAP, SEGAP, FFILE, KGRID, SPN )
  ...
  OPEN(UNIT=85, FILE=TRIM(FFILE), STATUS='REPLACE', ACTION='WRITE')
    ...
    END IF
    WRITE(85,'(F14.8)') EGAP(2)+0.5_DP*(EGAP(1)-EGAP(2))
    ...
    ELSE
      WRITE(85,'(F0.4)') EDELTA
    END IF
    WRITE(85,'(F14.8)') SEGAP(2)+0.5_DP*(SEGAP(1)-SEGAP(2))
...
SUBROUTINE WRITE_GRID(OUTGRID,KGRID)
  ...
  DIST1=1.2_DP*MAX(PAR%FBORDER,PAR%WDIF)
  DIST2=DIST1/4.0_DP
  DIST1=-DIST1
...
SUBROUTINE FERMISURFACE(TEBS,OROOTS,ROOTSXYZ,KGRID,LATTICE,BANDSPEC,FERMIFILE,SPECFILE,FILETYPE,SPN)
 ...
 CHARACTER(LEN=40) :: FFILE,SFILE,BFILE
 CHARACTER(LEN=11) :: OCHAR
 CHARACTER(LEN=1) :: CNUM
 REAL(DP),PARAMETER :: THIN=0.0000001_DP
 REAL(DP), DIMENSION(:,:), INTENT(IN) :: ROOTSXYZ
 REAL(DP), DIMENSION(:,:), ALLOCATABLE :: FERMIXY,FSU
 REAL(DP), DIMENSION(:), ALLOCATABLE :: XVEC
 REAL(DP) :: FXY0,WSUM,OSUM
 INTEGER,DIMENSION(2) :: RES
 INTEGER :: XRES, YRES, PROG, HPROG,SPN
 INTEGER, DIMENSION(3) :: VEC
 REAL(DP),DIMENSION(3,3) :: PX,LWORK
 REAL(DP),DIMENSION(2,3) :: REALSYMP
 REAL(DP),DIMENSION(2,2) :: NMAT
 REAL(DP),DIMENSION(3) :: NVEC,RNVEC,VEC1,VEC2,AREAL,AREAL1,BREAL,BREAL1
 REAL(DP),DIMENSION(2) :: OVEC,LVEC,SVEC,FXY,XBORDER,YBORDER
 REAL(DP) :: XDIST,YDIST,XRANGE,YRANGE,D,DTEMP,AS,AO,ZMIN,ZMAX
 INTEGER :: I,J,K,KPT,P,RN,SIGN1,SIGN2,FILETYPE,CON,IOCHECK,ONUM
 LOGICAL,DIMENSION(N%ORB) :: NOORB
! print*,par%ireflect
...
!********************* setup Filenames ****************
 IF(PAR%SPN==2)THEN
   WRITE(CNUM,'(I1)') SPN
   FFILE='temp'//TRIM(FERMIFILE)//CNUM
   SFILE='temp'//TRIM(SPECFILE)//CNUM
   BFILE=TRIM(BANDSPEC)//CNUM
 ELSE
   FFILE='temp'//TRIM(FERMIFILE)
   SFILE='temp'//TRIM(SPECFILE)
   BFILE=TRIM(BANDSPEC)
 END IF
 ...
 DO I=1,RN
  IF(OROOTS(I)%LOCU)THEN
    ...
    IF(FILETYPE/=3)THEN
      IF(PAR%FORB==0)THEN
        DO J=1,N%ORB
          IF(OROOTS(I)%OC(J)>0.05_DP)THEN
            NOORB(J)=.FALSE.
            ONUM=ONUM+1
          END IF
        END DO
      ELSE
        IF(OROOTS(I)%OC(PAR%FORB)>0.05_DP) NOORB(PAR%FORB)=.FALSE.
      END IF
    END IF
    P=P+1
  END IF
...
!***************** figure out the border of the fermisurface ***********************
 ...
 DO P=1,N%PATH
   ...
   CGRID(P)%B=BREAL
   IF(P==1)THEN
     XBORDER(1)=MIN(AREAL(1),BREAL(1))
     XBORDER(2)=MAX(AREAL(1),BREAL(1))
     YBORDER(1)=MIN(AREAL(2),BREAL(2))
     YBORDER(2)=MAX(AREAL(2),BREAL(2))
     ZMIN=MIN(AREAL(3),BREAL(3))
     ZMAX=MAX(AREAL(3),BREAL(3))
   ELSE
     XBORDER(1)=MIN(XBORDER(1),AREAL(1),BREAL(1))
     XBORDER(2)=MAX(XBORDER(2),AREAL(1),BREAL(1))
     YBORDER(1)=MIN(YBORDER(1),AREAL(2),BREAL(2))
     YBORDER(2)=MAX(YBORDER(2),AREAL(2),BREAL(2))
     ZMIN=MIN(ZMIN,AREAL(3),BREAL(3))
     ZMAX=MAX(ZMAX,AREAL(3),BREAL(3))
   END IF
 END DO
 PROG=1+N%PATH
...
!****************** get surface points for symmetry operation ************************
 PX(1,:)=KGRID(1)%A(:)
 PX(2,:)=KGRID(1)%B(:)

 IF(( KDISTANCE(KGRID(1)%A,KGRID(N%PATH)%A,LATTICE%B) > 0.001_DP ).AND. &
 &      (KDISTANCE(KGRID(1)%B,KGRID(N%PATH)%A,LATTICE%B) > 0.001_DP))THEN
   PX(3,:)=KGRID(N%PATH)%A(:)
 ELSE
   IF((KDISTANCE(KGRID(1)%A,KGRID(N%PATH)%B,LATTICE%B) > 0.001_DP ).AND. &
   &     (KDISTANCE(KGRID(1)%B,KGRID(N%PATH)%B,LATTICE%B) > 0.001_DP))THEN
       PX(3,:)=KGRID(N%PATH)%B(:)
   ELSE
      DO P=2,N%PATH-1
        IF((KDISTANCE(KGRID(1)%A,KGRID(P)%A,LATTICE%B) > 0.001_DP ).AND. &
        &     (KDISTANCE(KGRID(1)%B,KGRID(P)%A,LATTICE%B) > 0.001_DP))THEN
           PX(3,:)=KGRID(P)%A(:)
           EXIT
        END IF
      END DO
      IF(P==N%PATH)THEN
        DO P=2,N%PATH-1
          IF((KDISTANCE(KGRID(1)%A,KGRID(P)%B,LATTICE%B) > 0.001_DP ).AND. &
          &     (KDISTANCE(KGRID(1)%B,KGRID(P)%B,LATTICE%B) > 0.001_DP))THEN
            PX(3,:)=KGRID(P)%B(:)
            EXIT
          END IF
        END DO
        IF(P==N%PATH)THEN
          WRITE(*,'(A)') "WARNING: Couldn't find k-points on one surface"
        END IF
      END IF
   END IF         
 END IF
 ...
 IF(N%FIT(SPN)>0)THEN
 DO I=1,N%FIT(SPN)
   IF(ABS(ZMAX-ZMIN)>THIN)THEN
           
!           PRINT*,'wrong'
     FERMIXY(I,1)=SIGN(SQRT(ROOTSXYZ(I,1)**2+ROOTSXYZ(I,3)**2),ROOTSXYZ(I,1))
     FERMIXY(I,2)=SIGN(SQRT(ROOTSXYZ(I,2)**2+ROOTSXYZ(I,3)**2),ROOTSXYZ(I,2))
   ELSE
!           PRINT*,'WRITE RIGHT'
     FERMIXY(I,1)=ROOTSXYZ(I,1)
     FERMIXY(I,2)=ROOTSXYZ(I,2)
!     write(*,'(A,2F8.4)') 'fermixy =',FERMIXY(I,:)
   END IF
END DO
END IF
!print*,'X::minmax =',minval(FERMIXY(:,1)),maxval(FERMIXY(:,1))
!print*,'Y::minmax =',minval(FERMIXY(:,2)),maxval(FERMIXY(:,2))

   CALL CHANGE_BASIS(LATTICE%B,(KGRID(1)%B-KGRID(1)%A),VEC1)
   CALL CHANGE_BASIS(LATTICE%B,(PX(3,:)-PX(1,:)),VEC2)
   CALL CHANGE_BASIS(LATTICE%B,PX(1,:),AREAL)
   CALL CHANGE_BASIS(LATTICE%B,PX(3,:),AREAL1)
!   CALL CHANGE_BASIS(LATTICE%B,NVEC,RNVEC)

!**************** Check the Symmetry option *********************
 IF(PAR%IREFLECT>1)THEN
   DO I=1,2
     IF(PAR%SYMREC)THEN
      CALL CHANGE_BASIS(LATTICE%B,PAR%SYMPOINTS(I,:),REALSYMP(I,:))
     ELSE
      REALSYMP(I,:)=PAR%SYMPOINTS(I,:)
      LWORK=LATTICE%B
      CALL INVERT_MATRIX(LWORK)
      CALL CHANGE_BASIS(LWORK,REALSYMP(I,:),PAR%SYMPOINTS(I,:))
!      write(*,'(a,3f14.7)')'parsympoints ==  ',PAR%SYMPOINTS(I,:)
     END IF
   END DO
   IF(ABS(ZMAX-ZMIN)>THIN)THEN
       OVEC(1)=SIGN(SQRT(REALSYMP(1,1)**2+REALSYMP(1,3)**2),REALSYMP(1,1))
       OVEC(2)=SIGN(SQRT(REALSYMP(1,2)**2+REALSYMP(1,3)**2),REALSYMP(1,2))
   ELSE
       OVEC(1)=REALSYMP(1,1)
       OVEC(2)=REALSYMP(1,2)
   END IF
...
!   CALL EXPRO(NVEC,VEC1,VEC2)
!   CALL EXPRO(RNVEC,(PX(2,:)-PX(1,:)),(PX(3,:)-PX(1,:)))
   CALL EXPRO(RNVEC,(PX(2,:)-PX(1,:)),(PX(3,:)-PX(1,:)))
!   call mat_print(px)
!   call mat_print(lattice%b)
! CALL SOLVE_ES(PX,(/0.0_DP,0.0_DP,0.0_DP/),NVEC)
! print*,'nvec ==  ',nvec
! print*,'rnvec ==  ',rnvec
     DO P=1,3

!   write(*,'(a,f14.7)')'dp ==  ',rnvec(1)*px(p,1)+&
!    & rnvec(2)*px(p,2)+ rnvec(3)*px(p,3)        
       DTEMP=RNVEC(1)*PX(P,1)+RNVEC(2)*PX(P,2)+RNVEC(3)*PX(P,3)
       IF(P>1)THEN
         IF(ABS(D-DTEMP)>0.00001_DP)THEN
           D=0.0_DP
           EXIT
         END IF
       ELSE
     ...
     END DO
!       print*,'D = ',d 
!       print*,'realsymp = ',realsymp(1,:)
!   refl: DO I=1,PAR%IREFLECT/2
!     IF (ABS(DOT_PRODUCT(REALSYMP(1,:),NVEC)-D)>0.001_DP)THEN
     IF (ABS(DOT_PRODUCT(PAR%SYMPOINTS(1,:),RNVEC)-D)>0.001_DP)THEN
       CALL CLEAN_DISPLAY_LINE()

       WRITE(*,'(A)') 'WARNING: The 1st symmetrypoint from the INPAR file'
       WRITE(*,'(A)') '             cartesian                 ||              reciprocal'
       WRITE(*,'(3F13.7,A4,3F13.7)') PAR%SYMPOINTS(1,:),' || ',REALSYMP(1,:)
...
!     IF (ABS(DOT_PRODUCT(REALSYMP(2,:),NVEC)-D)>0.00001_DP)THEN
     IF (ABS(DOT_PRODUCT(PAR%SYMPOINTS(2,:),RNVEC)-D)>0.00001_DP)THEN
       CALL CLEAN_DISPLAY_LINE()
       WRITE(*,'(A)') 'WARNING: The 2nd symmetrypoint from the INPAR file'
       WRITE(*,'(A)') '             cartesian                 ||              reciprocal'
       WRITE(*,'(3F13.7,A4,3F13.7)') PAR%SYMPOINTS(2,:),' || ',REALSYMP(2,:)
       WRITE(*,*) 'is not an element of the surface'
...
!       EXIT refl
   ...
   IF(PAR%IREFLECT==2)THEN
     IF(ABS(KDISTANCE(PAR%SYMPOINTS(2,:),PAR%SYMPOINTS(1,:),LATTICE%B))<0.000001_DP)THEN
       CALL CLEAN_DISPLAY_LINE()
 ...
 IF(PAR%IREFLECT>1)THEN
!   VEC1=REALSYMP(2,:)-REALSYMP(1,:)
!   D=DOT_PRODUCT(VEC1)
!   CALL EXPRO(BREAL,(REALSYMP(1,:)-AREAL),VEC1)
!   OVEC(1)=SQRT(DOT_PRODUCT(BREAL,BREAL))/SQRT(DOT_PRODUCT(VEC1,VEC1))
!   CALL EXPRO(BREAL1,(REALSYMP(1,:)-AREAL1),VEC1)
!   XRANGE=SQRT(DOT_PRODUCT(BREAL1,BREAL1))/SQRT(DOT_PRODUCT(VEC1,VEC1))
!   print*,'ovec :: xrange :: ',ovec(1),xrange
!   IF(XRANGE<OVEC(1))THEN
!     FERMIXY(:,1)=OVEC(1)-FERMIXY(:,1)
!     OVEC(1)=XRANGE
!     AREAL=AREAL1
!     BREAL=BREAL1
!     VEC2=-VEC2
!   END IF
!   CALL EXPRO(BREAL,(REALSYMP(1,:)-AREAL),VEC2)
!   OVEC(2)=SQRT(DOT_PRODUCT(BREAL,BREAL))/SQRT(DOT_PRODUCT(VEC2,VEC2))

   IF(PAR%IREFLECT==2)THEN
     CALL EXPRO(BREAL,(REALSYMP(2,:)-AREAL),VEC1)
     LVEC(1)=SQRT(DOT_PRODUCT(BREAL,BREAL))/SQRT(DOT_PRODUCT(VEC1,VEC1))
 
     CALL EXPRO(BREAL,(REALSYMP(2,:)-AREAL),VEC2)
     LVEC(2)=SQRT(DOT_PRODUCT(BREAL,BREAL))/SQRT(DOT_PRODUCT(VEC2,VEC2))
!print*,'lvec == ',lvec
     LVEC=LVEC-OVEC
     FORALL(J=1:2) NMAT(J,1)=LVEC(J)
     IF(ABS(LVEC(1))>0.000001_DP)THEN
        NMAT(1,2)=LVEC(2)
        NMAT(2,2)=-LVEC(1)
     ELSE
        NMAT(1,2)=-LVEC(2)
...
! call mat_print(nmat)
! ovec=(/0.0,0.0/)
!print*,'for paths',par%ireflect
!print*,'ireflect =',par%ireflect

!***************** adjust the borders due to symmetry operations **************************
IF(PAR%IREFLECT==4)THEN
   XDIST=MAX(ABS(OVEC(1)-XBORDER(1)),ABS(OVEC(1)-XBORDER(2)))
   XBORDER(1)=OVEC(1)-XDIST
   XBORDER(2)=OVEC(1)+XDIST
   YDIST=MAX(ABS(OVEC(2)-YBORDER(1)),ABS(OVEC(2)-YBORDER(2)))
   YBORDER(1)=OVEC(2)-YDIST
   YBORDER(2)=OVEC(2)+YDIST
ELSE IF (PAR%IREFLECT==2)THEN
   CALL SOLVE_ES(NMAT,(/ XBORDER(2),YBORDER(2) /)-OVEC(:),XVEC)
   SVEC=OVEC+XVEC(1)*LVEC
   XBORDER(1) = 2.0_DP*SVEC(1) - XBORDER(2)
   YBORDER(1) = -1.0_DP*( 2.0_DP*SVEC(2) - YBORDER(2) )
   DEALLOCATE(XVEC)
!ELSE
!   XBORDER(1)=AREAL(1)
!   YBORDER(1)=AREAL(2)
...
!print*,'yborder =',yborder
 ...
 IF((PAR%SPECFUN).AND.(N%SPEC(SPN)>0))THEN
    OPEN(UNIT=201,FILE=TRIM(SFILE),STATUS='REPLACE',ACTION='WRITE')
!    IF(FILETYPE/=3) WRITE(201,'(A)') '#ORBITAL='//TRIM(ADJUSTL(PAR%ORBNAMES(PAR%FORB)))
!    IF(NOORB)WRITE(201,'(A)') '#noorbital'
    ...
    DO I=1,N%SPEC(SPN)
         ...
         YRANGE=KDISTANCE(KGRID(P)%A,KGRID(P)%B,LATTICE%B)
         VEC1=KGRID(P)%B-KGRID(P)%A
         VEC1=KGRID(P)%A+(YDIST/YRANGE)*VEC1
         CALL CHANGE_BASIS(LATTICE%B,VEC1,AREAL)
         IF(ABS(ZMAX-ZMIN)>THIN)THEN
            FXY(1)=SIGN(SQRT(AREAL(1)**2+AREAL(3)**2),AREAL(1))
            FXY(2)=SIGN(SQRT(AREAL(2)**2+AREAL(3)**2),AREAL(2))
         ELSE
            FXY(1)=AREAL(1)
            FXY(2)=AREAL(2)
         END IF
      ...
      IF(PAR%IREFLECT==4)THEN
        FXY(1)=OVEC(1)+SIGN1*(FXY(1)-OVEC(1))
        FXY(2)=OVEC(2)+SIGN2*(FXY(2)-OVEC(2))
      ELSE IF((PAR%IREFLECT==2).AND.(K==2))THEN
        CALL SOLVE_ES(NMAT,FXY(:)-OVEC(:),XVEC)
        SVEC=OVEC+XVEC(1)*LVEC
        FXY= 2.0_DP*SVEC - FXY
        FXY(2)=-FXY(2)
        DEALLOCATE(XVEC)
...
!         FXY(2)=FERMIXY(I,2)
      END IF
!      IF(ABS(FXY0-FXY(1))>0.000001_DP)WRITE(211,*)
!      FXY0=FXY(1)
      WRITE(201,'(4F18.12)') FXY(1), FXY(2), AS, AO
!      WRITE(211,'(4F13.8,3I3)') FXY(1), FXY(2), TORBCHAR(I)
    END DO
    SIGN2=-1*SIGN2
...
IF(N%FIT(SPN)>0)THEN
 ...
 OPEN(UNIT=211,FILE=TRIM(FFILE),STATUS='REPLACE',ACTION='WRITE')
  IF(FILETYPE/=3)THEN
    ...
    WRITE(211,'(A,I3,",",I3)') '#LAYOUT=',GET_LAYOUT(K)
  END IF
  ...
  DO K=1,PAR%IREFLECT
    DO I=1,N%FIT(SPN)
      ...
      CALL WRITE_PROGRESS(PROG,HPROG)
      IF(PAR%IREFLECT==4)THEN
        FXY(1)=OVEC(1)+SIGN1*(FERMIXY(I,1)-OVEC(1))
        FXY(2)=OVEC(2)+SIGN2*(FERMIXY(I,2)-OVEC(2))
      ELSE IF((PAR%IREFLECT==2).AND.(K==2))THEN
        CALL SOLVE_ES(NMAT,FERMIXY(I,:)-OVEC(:),XVEC)
        SVEC=OVEC+XVEC(1)*LVEC
        FXY= 2.0_DP*SVEC - FERMIXY(I,:)
        FXY(2)=-FXY(2)
        DEALLOCATE(XVEC)
      ELSE
         FXY(1)=FERMIXY(I,1)
...
!    PROG=PROG+N%FIT
 ...
 CALL PRINT_FGRID(TRIM(FERMIFILE)//'.grid',XBORDER,YBORDER,PAR%FGRID)
...
SUBROUTINE SPECTRAL_FUNCTION(TEBS,BANDSPEC,FILETYPE,LSURFACE)
  TYPE(BAND),DIMENSION(:),INTENT(IN) :: TEBS
  ...
  INTEGER :: COL,MAXKPT
  LOGICAL :: LSURFACE
  !print*,'begin spectral function',lsurface,par%sigma
  IF(LSURFACE)THEN
  ...
  SPIN: DO SPN=1,PAR%SPN
    ...
    IF(PAR%SPN==2) WRITE(SPECFILE(SPECLEN:SPECLEN),'(I1)') SPN
    approx=0
    N%SPEC(SPN)=0

     OPEN(UNIT=UNI,FILE=TRIM(SPECFILE),RECL=300,status="unknown")
      kptOLD=1
      i=1
      KPOINTS: DO COL=1,SIZE(TEBS,1)
       IF ((MOD(COL,10)==0).OR.(COL==SIZE(TEBS,1))) &
      ...
      ENDDO KPOINTS
      CLOSE(UNI)
      IF((LSURFACE).AND.(N%SPEC(SPN)==0))THEN
        IF(PAR%SPN==2)THEN
          WRITE(*,'(A,I1)') 'No data of the spectral fermisurface found for spin=',SPN
        ELSE
          WRITE(*,'(A)') 'No data of the spectral fermisurface found'
        END IF
      END IF
    END DO SPIN
